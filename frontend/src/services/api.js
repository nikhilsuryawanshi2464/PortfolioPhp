...
export const analyticsAPI   = { trackPageView:(d)=>api.post('/analytics/pageview',d), trackProjectView:(id)=>api.post(`/analytics/project/${id}`), trackResumeDownload:(f)=>api.post('/analytics/resume',{format:f}), trackEvent:(d)=>api.post('/analytics/event',d), getDashboard:(p)=>api.get('/analytics/dashboard',{params:p}), getRealTime:()=>api.get('/analytics/realtime'), getVisitorsList:()=>api.get('/analytics/visitors') };
export const skillsAPI      = { getAll:()=>api.get('/skills'), create:(d)=>api.post('/skills',d), update:(id,d)=>api.put(`/skills/${id}`,d), delete:(id)=>api.delete(`/skills/${id}`), reorder:(d)=>api.put('/skills/reorder',d) };
